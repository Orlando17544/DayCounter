...
if (storage.getBoolean('hasBeenOpened') == undefined) {
	...
	storage.set('AF', JSON.stringify({countryCode: 'AF', days: '0', maximumDays: '0'}));
	storage.set('AX', JSON.stringify({countryCode: 'AX', days: '0', maximumDays: '0'}));
...
const App: () => Node = () => {
	...
	const [userData, setUserData] = useMMKVObject<UserData>('AF');
	...
	console.log(storage.getString(userData.countryCode));
	...
	return (
		...
		{Object.keys(COUNTRIES_DATA).map((key, index) =>
			<TouchableOpacity key={key} style={{paddingBottom: 8}} onPress={() => {setUserData(JSON.parse(storage.getString(key))); setModalVisibleCountries(false);}}>
		...
		<View style={{flexDirection: 'row', alignItems: 'center'}}>
		<Text style={styles.text}>{userData.days}/</Text>
		<TextInput
		style={[styles.text, {fontWeight: 'bold'} ]}
		value={userData.maximumDays}
		onChangeText={newMaximumDays => {storage.set(userData.countryCode, JSON.stringify({countryCode: userData.countryCode, days: userData.days, maximumDays: newMaximumDays})); setUserData({...userData, maximumDays: newMaximumDays});}}
		/>
		</View>
		<View>
